<!--
meta:
  title: Payments / Refunds / Bugs
  owner: Commerce Engineering
  status: active
  last_reviewed: 2026-04-17
  tags: [payments, refunds, bugs]
-->
> Hub: [[10-modules/payments/features/refunds/overview]]

# Refunds - Bugs

## Purpose
This file tracks known issues, recurring defects, and defect patterns for **Refunds**.

## Current watch list
- watch for refund amount drifting from return document refund_total

## Usage rule
- Record confirmed recurring defects here.
- Link bug fixes to [[changelog]] after release.
- Use this file during regression testing and production issue analysis.

## Related feature files
- [[10-modules/payments/features/refunds/overview]]
- [[edge-cases]]
- [[backend]]
- [[frontend]]
- [[changelog]]

## Recommended reading order
Read when fixing defects or verifying regression risk after changes.
