<!--
meta:
  title: Customers / Customer Addresses / Bugs
  owner: Commerce Engineering
  status: active
  last_reviewed: 2026-04-17
  tags: [customers, customer-addresses, bugs]
-->
> Hub: [[10-modules/customers/features/customer-addresses/overview]]

# Customer Addresses - Bugs

## Purpose
This file tracks known issues, recurring defects, and defect patterns for **Customer Addresses**.

## Current watch list
- watch for checkout editing the reusable address book when only order snapshot was intended

## Usage rule
- Record confirmed recurring defects here.
- Link bug fixes to [[changelog]] after release.
- Use this file during regression testing and production issue analysis.

## Related feature files
- [[10-modules/customers/features/customer-addresses/overview]]
- [[edge-cases]]
- [[backend]]
- [[frontend]]
- [[changelog]]

## Recommended reading order
Read when fixing defects or verifying regression risk after changes.
